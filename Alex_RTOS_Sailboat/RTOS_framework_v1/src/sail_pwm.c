...
#define PWM_MODULE_ACTUATOR		TC2

#define PWM_ACTUATOR_OUT_PIN	PIN_PA12E_TC2_WO0
#define PWM_ACTUATOR_OUT_MUX	MUX_PA12E_TC2_WO0

static struct tc_module pwm_timer;
static struct tc_module pwm_timer_actuator;
...
enum status_code PWM_init_actuator(void) {
	...
	// Set the counter size and clock source.
	config_tc.clock_source    = GCLK_GENERATOR_3;
	config_tc.counter_size    = TC_COUNTER_SIZE_8BIT;
	config_tc.wave_generation = TC_WAVE_GENERATION_NORMAL_PWM;
	config_tc.waveform_invert_output = TC_WAVEFORM_INVERT_CC0_MODE | TC_WAVEFORM_INVERT_CC1_MODE;
	...
	tc_init(&pwm_timer_actuator, PWM_MODULE_ACTUATOR, &config_tc);
	tc_enable(&pwm_timer_actuator);
...
enum status_code PWM_SetActuatorDuty(uint8_t duty) {
	tc_set_compare_value(&pwm_timer_actuator, PWM_actuator, duty);	
	return STATUS_OK;
};
